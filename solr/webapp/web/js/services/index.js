...
var parse_memory_value = function( value )
{
  if( value !== Number( value ) )
  {
    ...
    value = parseFloat( match[1] ) * Math.pow( 1024, units.indexOf( match[3].toUpperCase() ) );
  }
...
angular.module('solrServices').
  factory('IndexService', function($http) {
    var service = {
      "loadSystemData": function(base, callback) {
        $http.get(base + '/admin/info/system?wt=json', {}).
          error(function(data, status, headers, config) {callback(data, null)}).
          success(function(system, status, headers, config) {
            system.lucene.solrSpecVersion = system.lucene["solr-spec-version"];
            system.lucene.solrImplVersion = system.lucene["solr-impl-version"];
            system.lucene.luceneSpecVersion = system.lucene["lucene-spec-version"];
            system.lucene.luceneImplVersion = system.lucene["lucene-spec-version"];

            system.system.loadAverage = ( system.system.uptime || '' ).match( /load averages?: (\d+[.,]\d\d),? (\d+[.,]\d\d),? (\d+[.,]\d\d)/ );
            if (!system.system.loadAverage) {
              system.system.loadAverage = ["","",""];
            }
            var maxPhysical = parse_memory_value( system.system.totalPhysicalMemorySize );
            var totalPhysical = parse_memory_value( system.system.totalPhysicalMemorySize - system.system.freePhysicalMemorySize);
            system.system.physicalPercent = 100*totalPhysical/maxPhysical;
